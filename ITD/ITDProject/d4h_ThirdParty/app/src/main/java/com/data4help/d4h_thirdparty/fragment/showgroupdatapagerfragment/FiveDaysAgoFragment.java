...
import static com.data4help.d4h_thirdparty.Config.DAYFIVE;
...
public class FiveDaysAgoFragment extends Fragment implements Runnable{
    ...
    private HashMap<String, String> selectData() {
        if(TodayGroupFragment.sevenDaysOfARequest != null)
            return new HashMap<>(Objects.requireNonNull(TodayGroupFragment.sevenDaysOfARequest.get(DAYFIVE)));
        else if(TodayGroupSubFragment.sevenDaysOfARequest != null)
            return new HashMap<>(Objects.requireNonNull(TodayGroupSubFragment.sevenDaysOfARequest.get(DAYFIVE)));
        else if(TodaySingleUserFragment.sevenDaysOfARequest != null)
            return new HashMap<>(Objects.requireNonNull(TodaySingleUserFragment.sevenDaysOfARequest.get(DAYFIVE)));
        else if(TodaySingleSubUserFragment.sevenDaysOfARequest != null)
            return new HashMap<>(Objects.requireNonNull(TodaySingleSubUserFragment.sevenDaysOfARequest.get(DAYFIVE)));
        else
            return null;
    }
