...
public class TodayGroupSubFragment extends Fragment implements Runnable {
    ...
    public void run() {
        ...
        JSONObject groupData = HomeSubscribedRequestFragment.getGroupParams().get(HomeSubscribedRequestFragment.getButtonGroupId());
        sevenDaysOfARequest = new HashMap<>();
        try {
            JSONArray sevenDaysHealthParam = Objects.requireNonNull(groupData).getJSONArray("healthParametersSents");


            setContextParam(groupData.getJSONObject("requestAttributesSent"));
